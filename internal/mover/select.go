...
// validation (renderOp.applyMove). The source AST is never mutated here.
...
//     so renderOp.applyMove strips it from the source entirely on move;
...
// renderOp.applyMove, after validation, and only on move.
func selectValueSpecs(gd *ast.GenDecl, re *regexp.Regexp) ([]match, error) {
	...
	for _, pm := range per {
		...
		// trimmed from the original spec by renderOp.applyMove on move.
