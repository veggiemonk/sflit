...
// extraction is one match staged for the sink: the decl plus every comment
...
// match so renderOp.applyMove can splice synthetic specs out of the source
// group on move.
...
// file.Comments by renderOp.applyMove (move only, after validation) so they
// never print twice from a moved-from source.
func extractMatches(fset *token.FileSet, file *ast.File, matches []match) []extraction {
	if len(matches) == 0 {
		return nil
	}
	...
	for _, d := range file.Decls {
		if matchSet[d] {
			out = append(out, extraction{Decl: d, LeadComms: perDecl[d]})
		}
	}
	return out
}
...
// removed from the source (renderOp.applyMove) so they print exactly once.
