// Package mover implements the sflit declaration-moving pipeline.
//
...
// parsed to AST, matching declarations are selected, the op is validated
...
// runOnce executes one optimistic attempt: parse, select, op, render,
...
func runOnce(cfg Config) (Result, error) {
	...
	op := buildRenderOp(fset, sinkFset, cfg.Source, cfg.Sink, src, origSink, extracted, cfg.Move)
	op.Selection = selectionSummary(cfg)
	if err := validateRenderOp(op, origSink, src); err != nil {
		return Result{}, err
	}
	log.Info("validation passed")

	// Only now — after validation, and only on move — is the source AST
	// mutated. Selection, extraction, and op building are read-only on
	// the source, so a copy or a failed validation never alters it.
	op.applyMove()

	srcBytes, sinkBytes, err := renderFiles(op)
	if err != nil {
		return Result{}, err
	}
	...
	return Result{
		...
		Move:                  cfg.Move,
		Matched:               matched,
		DeclarationsRemaining: countNonImportDecls(op.SrcFile.Decls),
	}, nil
}
...
// toggles. extraction from RunCLI so the drift tests can cross-check the
...
func cliFlagSet(stderr io.Writer, cfg *Config, jsonOutput, debug *bool) *flag.FlagSet {
	...
	fs.IntVar(
		...
		"retries",
		defaultRetries,
		"max re-runs after a concurrent-write conflict (0 or negative uses the default)",
	)
