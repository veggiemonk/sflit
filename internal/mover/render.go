...
// The source is printed through op.Fset and post-processed by goimports.
// The sink is rendered as two segments: any pre-existing sink content
// (printed through op.SinkFset) followed by the moved-decls segment
// (printed through op.Fset). Each segment is printed with its own
...
func renderFiles(op renderOp) ([]byte, []byte, error) {
	srcBytes, err := printAndFormat(op.Fset, op.SrcFile, op.SrcPath)
	if err != nil {
		return nil, nil, fmt.Errorf("render source: %w", err)
	}

	movedBytes, err := printNode(op.Fset, op.MovedFile)
	if err != nil {
	...
	if op.SinkIsNew {
		...
		existing, err := printNode(op.SinkFset, op.OrigSink)
		if err != nil {
	...
	combined, err = carryNamedImports(op.SinkPath, combined, op.SrcFile, requiredBlankImports(op.extracted))
	if err != nil {
		return nil, nil, err
	}

	sinkBytes, err := imports.Process(op.SinkPath, combined, nil)
	if err != nil {
...
func requiredBlankImports(extracted []extraction) []string {
