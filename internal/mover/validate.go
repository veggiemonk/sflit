...
// validateRenderOp checks invariants before any file is written. origSink is the
// sink file as loaded from disk (may be nil); origSrc is the source file.
func validateRenderOp(op renderOp, origSink, origSrc *ast.File) error {
	// Empty selection: op's sink has no decls appended beyond the original.
	if len(op.SinkFile.Decls) == op.OrigSinkDeclCount {
		selection := op.Selection
		if selection == "" {
			selection = "selection criteria"
		}
		return fmt.Errorf("no declarations matched in %s for %s", op.SrcPath, selection)
	}
	// Package mismatch.
	if origSink != nil && origSink.Name.Name != origSrc.Name.Name {
		return fmt.Errorf(
			"sink %s has package %q, but source %s has package %q",
			op.SinkPath, origSink.Name.Name, op.SrcPath, origSrc.Name.Name,
		)
	}
	...
	sameF, err := sameFile(op.SrcPath, op.SinkPath)
	if err != nil {
		return err
	}
	if sameF {
	...
	same, err := sameDir(op.SrcPath, op.SinkPath)
	if err != nil {
		return err
	}
	...
	roles := []fileRole{{origSrc, op.SrcPath, "source"}}
	if origSink != nil {
		roles = append(roles, fileRole{origSink, op.SinkPath, "sink"})
	}
	for _, r := range roles {
	...
	if err := validateDirectives(op, same); err != nil {
		return err
	}
	if err := validateBuildConstraints(op); err != nil {
		return err
	}
	if err := validateImportAliases(op, origSink, origSrc); err != nil {
		return err
	}
	if err := validateNoStrandedRefs(op, same); err != nil {
		return err
	}
	...
	for i := range op.OrigSinkDeclCount {
		for _, k := range collisionKeys(op.SinkFile.Decls[i]) {
			existing[k] = true
		}
	}
	for i := op.OrigSinkDeclCount; i < len(op.SinkFile.Decls); i++ {
		for _, k := range collisionKeys(op.SinkFile.Decls[i]) {
			if existing[k] {
				return fmt.Errorf(
...
// sameDirHint is the precomputed sameDir result from validateRenderOp.
func validateDirectives(op renderOp, sameDirHint bool) error {
	embed, linkname := travellingDirectives(op.extracted)
	if !embed && !linkname {
		return nil
	}
	if sameDirHint {
...
func validateImportAliases(op renderOp, origSink, origSrc *ast.File) error {
	if origSink == nil {
		return nil
	}
...
// directory).
// sameDirHint is the precomputed sameDir result from validateRenderOp.
func validateNoStrandedRefs(op renderOp, sameDirHint bool) error {
	if sameDirHint || len(op.extracted) == 0 {
		return nil
	}
	...
	for _, d := range op.SrcFile.Decls {
		switch x := d.(type) {
		case *ast.FuncDecl:
			topLevel[x] = true
		case *ast.GenDecl:
	...
	travelling, travellingNames := op.travelSet()
...
func validateBuildConstraints(op renderOp) error {
	srcConstraints := buildConstraintLinesFromAST(op.SrcFile)
	if op.SinkIsNew {
	...
	sinkConstraints := buildConstraintLinesFromAST(op.OrigSink)
