...
func TestValidate_EmptySelection(t *testing.T) {
	...
	op := buildRenderOp(fset, nil, "src.go", "sink.go", src, nil, nil, false)
	if err := validateRenderOp(
		op,
		nil,
		src,
	); err == nil ||
...
func TestValidate_PackageMismatch(t *testing.T) {
	...
	if err := validateRenderOp(op, sink, src); err == nil ||
...
func TestValidate_NewSinkRejectsBuildConstrainedSource(t *testing.T) {
	...
	ex := extractMatches(fset, src, ms)
	op := buildRenderOp(fset, nil, "src.go", "sub/sink.go", src, nil, ex, true)
...
func TestValidate_ImportAliasCollision(t *testing.T) {
	...
	ms, _ := selectDecls(src, Config{Regex: "^Foo"})
	ex := extractMatches(fset, src, ms)
	op := buildRenderOp(fset, sinkFset, "src.go", filepath.Join("other", "sink.go"), src, sink, ex, true)
	err := validateRenderOp(op, sink, src)
...
func TestValidate_StrandedRefs(t *testing.T) {
	...
	for _, tc := range cases {
		t.Run(tc.name, func(t *testing.T) {
			...
			ex := extractMatches(fset, src, ms)
			op := buildRenderOp(fset, nil, "src.go", tc.sink, src, nil, ex, tc.move)
			err = validateRenderOp(op, nil, src)
			if tc.wantErr == "" {
				if err != nil {
					t.Fatalf("want valid op, got %v", err)
				}
				return
			}
...
func TestValidate_Collision(t *testing.T) {
	...
	ms, _ := selectDecls(src, Config{Regex: "^Foo"})
	ex := extractMatches(fset, src, ms)
	op := buildRenderOp(fset, nil, "src.go", "sub/sink.go", src, sink, ex, false)
	if err := validateRenderOp(
		op,
		sink,
		src,
...
func TestCollisionKeys_ParenthesizedReceiver(t *testing.T) {
	...
	// collision key, or duplicate methods slip past validateRenderOp.
...
func TestValidate_SameDirCopyRejected_RelativeVsAbsoluteSpelling(t *testing.T) {
	...
	op := buildRenderOp(fset, nil, "./src.go", abs, src, nil, ex, false)
	if err := validateRenderOp(op, nil, src); err == nil ||
...
func TestValidate_CrossDirCopyAllowed(t *testing.T) {
	...
	ms, _ := selectDecls(src, Config{Regex: "^Foo"})
	ex := extractMatches(fset, src, ms)
	op := buildRenderOp(fset, nil, "src.go", "sub/sink.go", src, nil, ex, false)
	if err := validateRenderOp(op, nil, src); err != nil {
...
// pipeline ordering (select/extract/buildRenderOp are read-only; mutation only
// happens in applyMove, which Run calls after validateRenderOp succeeds).
...
`)
	...
	ex := extractMatches(fset, src, ms)
	op := buildRenderOp(fset, nil, "src.go", "sink.go", src, sink, ex, true)
	if err := validateRenderOp(op, sink, src); err == nil ||
...
func TestValidate_DotImportSinkRejected(t *testing.T) {
	...
	ex := extractMatches(fset, src, ms)
	op := buildRenderOp(fset, sinkFset, "src.go", "sink.go", src, sink, ex, true)
	err := validateRenderOp(op, sink, src)
...
func TestValidate_CgoSinkRejected(t *testing.T) {
	...
	ms, _ := selectDecls(src, Config{Regex: "^Foo"})
	ex := extractMatches(fset, src, ms)
	op := buildRenderOp(fset, sinkFset, "src.go", "sink/sink.go", src, sink, ex, true)
	err := validateRenderOp(op, sink, src)
...
func TestValidate_GeneratedMarkerPastLine20(t *testing.T) {
	...
	op := buildRenderOp(fset, nil, "src.go", "sub/sink.go", file, nil, ex, true)
	err := validateRenderOp(op, nil, file)
	if err == nil || !strings.Contains(err.Error(), "generated") {
...
func TestValidate_GeneratedMidSentenceNotRejected(t *testing.T) {
	...
	ex := extractMatches(fset, file, ms)
	op := buildRenderOp(fset, nil, "src.go", "sub/sink.go", file, nil, ex, true)
	if err := validateRenderOp(op, nil, file); err != nil {
