...
// renderOp holds the inputs of one split operation, ready to render.
//
// SrcFile is the source AST exactly as parsed. buildRenderOp never mutates it;
// on move, matched decls/specs and travelling comments are spliced out by
// [renderOp.applyMove], which Run calls only after validateRenderOp succeeds — a
...
// applyMove after validateRenderOp, so a validation failure leaves the parsed
// source untouched.
func (p *renderOp) applyMove() {
	if !p.Move {
		return
	}
...
func (p renderOp) travelSet() (travelling map[any]bool, travellingNames map[*ast.ValueSpec]map[string]bool) {
