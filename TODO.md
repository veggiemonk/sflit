...
- **Kind:** deferred improvement (decided).
...
- **Decision (2026-06-12):** promote-first flow. A release script/make target verifies Unreleased is non-empty, promotes it to `vX.Y.Z — date`, commits, tags. CI extracts that version's section and passes it to goreleaser via `--release-notes` (commit-based changelog generation disabled); a CI guard fails the release when the tagged version has no CHANGELOG heading. CHANGELOG.md is the single source of truth.
...
- **Kind:** deferred improvement (decided: inherit + suffix check).
- **Decision (2026-06-12):** new sinks inherit the source's explicit `//go:build` lines and the rejection branch only fires for existing sinks. Additionally close the pre-existing silent hole: validate filename-implied constraints (`_linux`, `_arm64`, `_test` suffixes) are compatible between source and sink — `foo_linux.go` (no explicit line) → `bar.go` must be rejected, for both new and existing sinks. Behavior change is the backward-compatible direction (rejection → success); help/schema/CHANGELOG updated, golden refreshed.
...
- **Acceptance:** new sinks receive the source's constraint lines, the rejection branch only fires for existing sinks, filename-implied constraint mismatches are rejected with a red test per direction, help/schema/CHANGELOG updated, golden refreshed.
...
- **Why deferred:** out of the approved scope for this pass.
- **Decisions (2026-06-12):** the three decision-blocked items are settled. (1) **Symlink policy:** resolve, don't refuse — canonicalize via `filepath.EvalSymlinks` on parent dir + base at plan/lock time so writes, hashes, lock order, and lock identity all use the real path; hard-link aliases stay a documented out-of-scope limitation. (2) **JSON error contract:** when `-json` is set and the run fails, emit a JSON error object on stdout (`{"error", "kind", "attempts"}`) with the `kind` taxonomy documented in help + tool-schema and golden-pinned. (3) **Renames:** both — **done 2026-06-12** on `docs/adr-0001-parallel-editing`: `Plan` → `renderOp` (plan.go → renderop.go), `internal/splitter` → `internal/mover` (splitter.go → mover.go), `Match`/`SpecOrigin`/`Extracted` unexported as `match`/`specOrigin`/`extraction`, glossary anchors the code names. `SemEqual` deliberately left exported — its keep-vs-unexport hangs on the possible `-verify` CLI feature (separate finding, still open).
